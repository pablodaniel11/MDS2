...
public class Administrador extends Moderador implements Serializable {
	...
	private java.util.Set this_getSet (int key) {
		if (key == ORMConstants.KEY_ADMINISTRADOR_SON_RECIBIDOS) {
			return ORM_son_recibidos;
		}
		else if (key == ORMConstants.KEY_ADMINISTRADOR_ES_CREADA) {
	...
	@JoinTable(name="`Ticket_Administrador`", joinColumns={ @JoinColumn(name="`AdministradorUsuarioID`") }, inverseJoinColumns={ @JoinColumn(name="`TicketIdticket`") })	
	...
	@Transient	
	public final TicketSetCollection son_recibidos = new TicketSetCollection(this, _ormAdapter, ORMConstants.KEY_ADMINISTRADOR_SON_RECIBIDOS, ORMConstants.KEY_TICKET_RECIBE, ORMConstants.KEY_MUL_MANY_TO_MANY);
