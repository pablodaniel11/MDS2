...
public class AdministradorDAO {
	...
	public static boolean deleteAndDissociate(Administrador administrador)throws PersistentException {
		try {
			Ticket[] lSon_recibidoss = administrador.son_recibidos.toArray();
			for(int i = 0; i < lSon_recibidoss.length; i++) {
				lSon_recibidoss[i].recibe.remove(administrador);
			}
			Seccion[] lEs_creadas = administrador.es_creada.toArray();
			for(int i = 0; i < lEs_creadas.length; i++) {
				lEs_creadas[i].setCrea(null);
			}
			Ticket[] lTickets_moderadors = administrador.tickets_moderador.toArray();
			...
			Ticket[] lTickets_usuarios = administrador.tickets_usuario.toArray();
